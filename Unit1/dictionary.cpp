// Consider the Dictionary Implementations which allow for efficientstorage and retrieval of key - value pairs using binary search trees.Eachnode in the tree stores a(key, value) pair.The dictionary should support  the following operations efficiently :
...
public:
    ...
    void search(string target_word)
    {
        ...
        while (curr != NULL)
        {
            ...
            {
                cout << "Found -> " << curr->word << ": " << curr->meaning
                     << endl;
                return;
            }
    ...
    node *copyDictionary(node *original_root)
    {
        if (original_root == NULL)
            return NULL;
        node *copy_root = new node(original_root->word,
                                   original_root->meaning);
        ...
        while (!q_orig.empty())
        {
            ...
            if (curr_orig->left)
            {
                node *L = new node(curr_orig->left->word,
                                   curr_orig->left->meaning);
                curr_copy->left = L;
            ...
            if (curr_orig->right)
            {
                node *R = new node(curr_orig->right->word,
                                   curr_orig->right->meaning);
                curr_copy->right = R;
